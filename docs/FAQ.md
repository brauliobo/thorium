## Alacrium FAQ <img src="https://github.com/brauliobo/alacrium/blob/main/logos/STAGING/faq.png" width="48">
...
__<a id="1"></a>1. Q: Why is the Android version not letting me sign in to Google Sync?__
 ...
 devices, opening Alacrium will cause a notification saying "Alacrium won't run without Google Play Services, which are not supported by your
 device". This is baloney, and it will run just fine.

__<a id="2"></a>2. Q: Why are Netflix, Spotify, Twitch, etc. not playing?__
 __A:__ Alacrium includes Widevine, which is a proprietary component used to play encrypted media streams on sites like these. It is used to prevent
 ...
 high resolution content. So, the end result is that Alacrium will play Widevine encrypted content on Linux, but at lower quality, and on Windows or MacOS
 ...
 registered applications, Alacrium will be restricted in this way (as are most FOSS Chromium and Firefox forks).

__<a id="3"></a>3. Q: Why is Alacrium always behind Chromium/Chrome as far as the major release number?__
 __A:__ Chromium/Chrome used to be on a 6 week release schedule, but then they moved to a 4 week release schedule. Rebasing Alacrium is a long, hard, convoluted process
 ...
 Building Alacrium takes alot of resources. The Chromium repository is 100+ GB in size, and compiling requires 8GB of free RAM or more. Compiling Alacrium for Linux on
 ...
 or something privately relayed to me as per the [SECURITY.md](https://github.com/brauliobo/alacrium/blob/main/SECURITY.md) file), I try my best to fix/update it and get
 releases out as soon as possible. Another thing that people keep
 suggesting is to use some kind of CI build service. This wouldn't help with the rebasing/development time, and because of the amount of resources needed to compile Alacrium,
...
__<a id="4"></a>4. Q: Why isn't Alacrium UnGoogled?__
 __A:__ Alacrium integrates many patches from UnGoogled Chromium (see [PATCHES.md](https://github.com/brauliobo/alacrium/blob/main/docs/PATCHES.md)), but one of the project
 ...
 instances of multiple Chromium-based browsers. Thus, while Alacrium has removed telemetry and field trials, there are instances where Alacrium still connects to Google servers, even
 ...
 Google without any user interaction is minimal, and contains no personally identifiable information (PII) that could be used to identify you. Alacrium is open to taking requests for
 ...
 If you feel that you are missing out since UnGoogled Chromium does not have the performance-enhancing compiler optimizations of Alacrium, then one could compile it for themselves
 following the instructions [Here](https://github.com/ungoogled-software/ungoogled-chromium/blob/master/docs/building.md), and using the changes in Alacrium's [//build directory](https://github.com/brauliobo/alacrium/tree/main/src/build) as a starting point for adding the compiler optimizations to UnGoogled's build config. I am
 willing to provide guidance or help with compiling Chromium or integrating Alacrium's compiler optimization patches to an existing source code checkout.

__<a id="5"></a>5. Q: How can I build Alacrium for myself?__
 __A:__ See the documentation for various platforms here > https://alacrium.rocks/docs/
