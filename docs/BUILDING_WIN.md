# Checking out and Building Alacrium for Windows &nbsp;<img src="https://github.com/brauliobo/alacrium/blob/main/logos/NEW/build_light.svg#gh-dark-mode-only" width="48"> <img src="https://github.com/brauliobo/alacrium/blob/main/logos/NEW/build_dark.svg#gh-light-mode-only" width="48">
...
All the following script instructions assume that Chromium is in *C:\src\chromium\src* and that you have changed to the alacrium repo in bash.
...
Chromium & Alacrium use [Ninja](https://ninja-build.org) as its main build tool along with
...
We will be copy/pasting the contents of the [win_args.gn](https://github.com/brauliobo/alacrium/blob/main/win_args.gn) file (from *C:\src\Alacrium\win_args.gn*) into notepad.
...
* For other build arguments, and what the ones that Alacrium uses do, see [ABOUT_GN_ARGS.md](https://github.com/brauliobo/alacrium/blob/main/docs/ABOUT_GN_ARGS.md) & [win_args.list](https://github.com/brauliobo/alacrium/blob/main/infra/win_args.list)
...
Build Alacrium (the "chrome" target), and the other things like [chromedriver](https://chromedriver.chromium.org/home) and [alacrium_shell](https://github.com/brauliobo/alacrium/tree/main/alacrium_shell#readme) with Ninja using the command:

```shell
autoninja -C out\alacrium alacrium chromedriver alacrium_shell setup mini_installer -j8
```
...
`gn ls out\alacrium` from the command line. To compile one, pass to Ninja
the GN label with no preceding "//" (so for `//chrome/test:unit_tests`
use autoninja -C out\alacrium chrome/test:unit_tests).

## Install/Run Alacrium

Once it is built, you can simply run the browser:

```shell
out\alacrium\alacrium.exe
```

Or install it using the mini_installer:

```shell
out\alacrium\mini_installer.exe
```
...
To update an existing Alacrium checkout, just download the latest .zip, or do a git pull:
