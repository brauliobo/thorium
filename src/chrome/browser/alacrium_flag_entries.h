...
#ifndef CHROME_BROWSER_ALACRIUM_FLAG_ENTRIES_H_
#define CHROME_BROWSER_ALACRIUM_FLAG_ENTRIES_H_
    ...
    {"auto-dark-mode",
     "GTK Auto Dark Mode",
     "Enables Alacrium to automatically change to Dark Mode according to the system GTK Theme.",
     kOsLinux, SINGLE_VALUE_TYPE("auto-dark-mode")},
#endif // BUILDFLAG(IS_LINUX)

    // alacrium-2024 + restore-tab-button: require the classic UI patch, not ported to M147.
    ...
    {"force-disable-tab-outlines",
     ...
     "Force disables tab outline strokes. Alacrium enables them by default, improving accessiblity in dark mode, incognito mode, and low contrast themes.",
     kOsDesktop, SINGLE_VALUE_TYPE("force-disable-tab-outlines")},
    {"disable-alacrium-icons",
     "Disable Alacrium Top Bar Icons",
     "Disables the custom colored top bar icons in Alacrium, and restores the default grey Chromium icon colors.",
     kOsDesktop, SINGLE_VALUE_TYPE("disable-alacrium-icons")},
    {"left-aligned-tab-search-button",
     "Left Aligned Tab Search Button",
     "The default placement of the tab search button was changed to the left of the tabstrip in M125, but Alacrium has reverted this back "
...
//#if BUILDFLAG(IS_WIN)
    //{"transparent-tabs",
     //"Alacrium Semi-Transparent Tabs UI",
...
//#endif // BUILDFLAG(IS_WIN)
    ...
    {"disable-alacrium-dns-config",
     "Disable Alacrium Custom DNS Config",
     "Disables the custom DNS configuration used by default in Alacrium. Useful when this config breaks something, "
     "due to external apps or a non-standard system DNS config setting.",
     kOsDesktop, SINGLE_VALUE_TYPE("disable-alacrium-dns-config")},
    //{"side-panel-journeys",
     //"Side Panel Journeys",
    ...
    {"force-high-contrast",
     ...
     "Enables high contrast mode for all Alacrium instances.",
    ...
    {"keep-all-history",
     "Keep All History",
     "Retain All local browsing history. By default history older than 4 months is expired and purged. Alacrium flag",
    ...
    // depend on inherited media/content patches not ported to M147.
    {"gtk-version",
    ...
    {"gpu-no-context-lost",
     "No GPU Context Lost",
     "Inform Alacrium's GPU process that a GPU context will not be lost in power saving mode, screen saving mode, etc. "
    ...
    {"disable-encryption",
     ...
     "This is used to enable portable user data directories. Enabled for Alacrium Portable.",
     kOsDesktop, SINGLE_VALUE_TYPE("disable-encryption")},
    {"disable-machine-id",
     "Disable Machine ID",
     "Disables use of a generated machine-specific ID to lock the user data directory to that machine. This is used to enable portable user data directories. Enabled for Alacrium Portable.",
    ...
    {"revert-from-portable",
     ...
     "When moving an Alacrium user profile from one drive to another (or one system to another), enable this flag before moving the profile directory. It should also be used "
...
#endif  // CHROME_BROWSER_ALACRIUM_FLAG_ENTRIES_H_
