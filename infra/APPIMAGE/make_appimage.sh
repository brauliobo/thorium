...
displayHelp () {
	printf "\n" &&
	printf "${bold}${GRE}Script to build Alacrium .AppImage on Linux.${c0}\n" &&
...
cp -r -v ./files/alacrium-shell ./temp/ &&
rm -r -v ./temp/cron &&
rm -r -v ./temp/alacrium-browser &&

printf "\n" &&
printf "${YEL}Building .AppImage using Alacrium.yml...\n" &&
printf "${c0}\n" &&
...
printf "\n" &&
printf "${GRE}Done! ${YEL}AppImage at //out/Alacrium*.AppImage\n" &&
printf "\n" &&
tput sgr0
