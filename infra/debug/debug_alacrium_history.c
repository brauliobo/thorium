// Open chrome://history in Alacrium and sample Alacrium process CPU usage.
//
// Build:
...
//   /tmp/debug_alacrium_history --profile "Profile 6"
//   /tmp/debug_alacrium_history --monitor-only
...
static int is_alacrium_cmd(const char *cmdline) {
  return strstr(cmdline, "alacrium") != NULL &&
         (strstr(cmdline, "/opt/alacrium-browser/") != NULL ||
          strstr(cmdline, "alacrium-browser") != NULL);
}
...
static int collect(Proc *procs, int max) {
  ...
  while ((ent = readdir(dir)) != NULL && count < max) {
    ...
    if (!is_alacrium_cmd(cmdline)) continue;
...
int main(int argc, char **argv) {
  const char *browser = "alacrium-browser";
  ...
  for (int sample = 1; sample <= samples; sample++) {
    ...
    if (!shown) printf("No Alacrium process above 1%% CPU.\n");
    fflush(stdout);
