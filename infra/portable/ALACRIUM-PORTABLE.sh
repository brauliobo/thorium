...
# after 30 days.
find "$(pwd)/.config/alacrium/Crash Reports/pending/" -mtime +30 \
...
# DESKTOP="alacrium-portable"

# Set XDG Title variable
TITLE="Alacrium Portable"

# Set the correct file name for the desktop file
export CHROME_DESKTOP="alacrium-portable.desktop"
...
export GNOME_DISABLE_CRASH_DIALOG=SET_BY_ALACRIUM

# Set config home.
XDG_CONFIG_HOME=${XDG_CONFIG_HOME:-$(pwd)/.config}

usage () {
  echo "alacrium-browser [-h|--help] [--temp-profile] [options] [URL]"
  echo
  ...
  MANWIDTH=80 man alacrium-browser | sed -e '1,/OPTIONS/d; /ENVIRONMENT/,$d'
  echo " See 'man alacrium-browser' for more details"
}
...
if [[ -f $XDG_CONFIG_HOME/alacrium/alacrium-flags.conf ]]; then
   CHROME_USER_FLAGS="$(cat $XDG_CONFIG_HOME/alacrium/alacrium-flags.conf)"
fi
...
exec -a "$0" "$HERE/alacrium" "--disable-machine-id" "--disable-encryption" "--user-data-dir=$PROFILE" "--disk-cache-dir=$CACHE" $CHROME_USER_FLAGS "$@"
