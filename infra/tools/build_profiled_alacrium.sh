...
OUT_DIR="${OUT_DIR:-out/alacrium-prof}"
...
nice -n 10 ionice -c2 -n7 autoninja -C "$OUT_DIR" alacrium -j"$JOBS"

cat <<EOF

Profiled Alacrium built at:
  $CR_SRC_DIR/$OUT_DIR/alacrium

Run it with an isolated profile:
  HOME=/tmp/alacrium-prof-home XDG_CONFIG_HOME=/tmp/alacrium-prof-home/.config \\
    $CR_SRC_DIR/$OUT_DIR/alacrium --user-data-dir=/tmp/alacrium-prof-profile \\
