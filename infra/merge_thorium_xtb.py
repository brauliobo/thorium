#!/usr/bin/env python3
"""Splice Thorium <translation> additions into Chromium's .xtb bundles.

For every file `src/chrome/app/resources/thorium_additions/<basename>.add`,
...
Invocation:
    python3 infra/merge_thorium_xtb.py $CR_SRC_DIR
"""
...
def merge(additions_dir, cr_src):
    ...
    print(f'merged thorium translations: {total_added} lines into {touched} files'
          f' ({skipped_missing} locales absent upstream)')


if __name__ == '__main__':
    if len(sys.argv) != 2:
        print('usage: merge_thorium_xtb.py <CR_SRC_DIR>', file=sys.stderr)
        sys.exit(1)
    here = os.path.dirname(os.path.abspath(__file__))
    additions = os.path.join(os.path.dirname(here), 'src/chrome/app/resources/thorium_additions')
